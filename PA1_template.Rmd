...
## unlink(temp)
```
This is the code that was implemented to unzip the file on Github.


```{r}
## https://github.com/AlisonDoucette/RepData_PeerAssessment1/blob/master/activity.zip
## exerData <- read.csv(unz(activity.zip,"activity.csv" ))
```
...
## Calculate Steps per Day
    stepsPerDay<-aggregate(exerData$Steps, by=list(exerData$dateExer), FUN=sum)
...
## Convert Date and Interval to YYYY-MM-DD HH:SS
    library(dplyr)
    exerData1 <- mutate(exerData, Hour = ExerInterval %/% 100, Minute = ExerInterval %% 100)  
    ## rm(exerData)
    exerData2 <-transform(exerData1,NewDate=paste0(dateExer," ",Hour,":",Minute,":","00"))
    library(lubridate)
